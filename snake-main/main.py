...
tail = Snake('snake.png', 350,225, 25,25,0)
snake = [head, tail]
food = Food(['snakefood.png','snakestrawberry.png','snakewatermelon.png'], -100, -100, 25,25)
...
while game:
    ...
    if head.rect.colliderect(food):
        head.eat(food)
        s = Snake('snakebody.png', head.rect.x, head.rect.y, 25,25, 0)
        snake.insert(1,s)
        if speed % 2 == 0:
            head.wait -=2
        if head.wait < 10:
    ...
    score_text = font1.render('Счёт:'+str(speed-1), 1,(255,255,255))
